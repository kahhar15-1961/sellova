...
import { CalendarClock, ChevronDown, Filter, Search, ShoppingBag } from 'lucide-react';
...
function initialForBuyer(email) {
...
export default function OrdersIndex({ header, rows, pagination, filters, index_url, status_options, summary }) {
    ...
    const toggleAll = (checked) => {
        setSelected(checked ? new Set((rows || []).map((row) => String(row.order ?? row.buyer))) : new Set());
    };
    ...
    const deleteOrder = (row) => {
        ...
        const label = String(row.order ?? 'this order');
                        ...
                        <table className="min-w-full border-collapse">
                            <thead>
                                <tr className="h-[45px] border-b border-slate-200 bg-slate-50/80 dark:border-slate-700 dark:bg-slate-900/30">
                                    ...
                                    <th className="min-w-[190px] px-3 text-left text-[10px] font-bold uppercase tracking-normal text-slate-500 dark:text-slate-400">Order</th>
                                    <th className="min-w-[260px] px-3 text-left text-[10px] font-bold uppercase tracking-normal text-slate-500 dark:text-slate-400">Buyer</th>
                                    <th className="min-w-[150px] px-3 text-left text-[10px] font-bold uppercase tracking-normal text-slate-500 dark:text-slate-400">Total</th>
                                    ...
                                    <th className="min-w-[190px] px-3 text-left text-[10px] font-bold uppercase tracking-normal text-slate-500 dark:text-slate-400">Placed</th>
                            ...
                            <tbody>
                                {rows?.length === 0 ? (
                                    ...
                                    (rows || []).map((row, index) => {
                                        const id = String(row.order ?? row.buyer ?? index);
                                        const buyer = String(row.buyer ?? '—');
                                        const buyerEmail = String(row.buyer_email ?? 'No email');
                                        return (
                                            <tr
                                                key={id}
                                            ...
                                            >
                                                ...
                                                <td className="px-3 align-middle">
                                                    <Link href={String(row.href || '#')} className="inline-flex items-center gap-2 text-[13px] font-bold text-slate-700 transition-colors hover:text-primary dark:text-slate-200">
                                                ...
                                                <td className="px-3 align-middle">
                                                    <div className="flex items-center gap-3">
                                                        <div className={cn('flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[11px] font-bold', avatarTone(buyerEmail || buyer))}>
                                                            {initialForBuyer(buyer || buyerEmail)}
                                                        </div>
                                                        <div className="min-w-0">
                                                            <span className="block truncate text-[13px] font-semibold text-slate-950 dark:text-slate-100">{buyer}</span>
                                                            <span className="block truncate text-[12px] font-medium text-slate-500 dark:text-slate-400">{buyerEmail}</span>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-3 align-middle text-[13px] font-semibold text-slate-700 dark:text-slate-300">
                                                    {formatMoney(row.gross_amount ?? row.total, row.currency, { currencyDisplay: 'code' })}
                                                </td>
                                                <td className="px-3 align-middle">
                                                ...
                                                <td className="px-3 text-right align-middle">
                                                    <DropdownMenu>
                                                        ...
                                                        <DropdownMenuContent align="end" className="min-w-[180px] rounded-lg">
                                                            <DropdownMenuLabel className="px-2 py-1.5 text-[11px] uppercase tracking-normal text-slate-500">Order actions</DropdownMenuLabel>
                                                            <DropdownMenuItem asChild className="rounded-md text-[13px]">
                                                                <Link href={String(row.href || '#')}>View order details</Link>
                                                            </DropdownMenuItem>
                                                            <DropdownMenuSeparator />
                                                            <DropdownMenuItem className="rounded-md text-[13px] text-red-600 focus:text-red-600 dark:text-red-400 dark:focus:text-red-400" onSelect={() => deleteOrder(row)}>
                                                                Delete order
                                                            </DropdownMenuItem>
                                                        </DropdownMenuContent>
                                                    </DropdownMenu>
