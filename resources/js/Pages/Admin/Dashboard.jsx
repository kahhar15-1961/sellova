...
/** @typedef {{ value: string | null, hint: string | null }} SummaryCell */
 ...
 *   summary: Record<string, SummaryCell>,
 *   recent_orders: object[],
 *   open_disputes: object[],
 *   pending_withdrawals: object[],
 *   seller_verification_queue: object[],
 *   product_moderation: object[],
 *   system_alerts: { severity: string, title: string, detail: string, href?: string }[],
 *   section_access: Record<string, boolean>,
    ...
    return (
        <AdminLayout className="bg-gradient-to-b from-slate-50 to-slate-100/80">
            ...
            <div className="mb-6 flex flex-col gap-1 rounded-xl border border-border/60 bg-card/80 px-4 py-3 shadow-sm backdrop-blur sm:flex-row sm:items-center sm:justify-between">
                ...
                <p className="text-xs text-muted-foreground">Figures refresh on each visit · read-only snapshot</p>
            </div>
                ...
                <TabsContent value="queues" className="mt-6 space-y-8">
                    <section>
                        <div className="mb-4 flex items-end justify-between gap-4">
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground">
                                    Prioritized slices — each loads at most eight rows with eager relations (no N+1).
                                </p>
                            </div>
                        </div>
                        <div className="grid gap-6 lg:grid-cols-2">
                            <DashboardQueuePanel
                                title="Recent orders"
                                description="Latest placed orders with buyer context."
                                href={links.orders}
                                locked={!sectionAccess.recent_orders}
                            >
                                {!recentOrders?.length ? (
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            {recentOrders.map((row) => (
                                                <TableRow key={row.id}>
                                                    <TableCell className="font-medium tabular-nums">{row.order_number}</TableCell>
                                                    <TableCell className="max-w-[140px] truncate text-muted-foreground">
                                                        {row.buyer_email ?? '—'}
                                                    </TableCell>
                                                    <TableCell className="tabular-nums">{fmtMoney(row.gross_amount, row.currency)}</TableCell>
                                                    <TableCell>
                                                        <StatusBadge status={row.status} />
                                                    </TableCell>
                                                    <TableCell className="text-muted-foreground">{fmtDate(row.placed_at)}</TableCell>
                                                </TableRow>
                                            ))}
                                        </TableBody>
                                    </Table>
                            ...
                            <DashboardQueuePanel
                                title="Open disputes"
                                description="Cases not yet resolved."
                                href={links.disputes}
                                locked={!sectionAccess.open_disputes}
                            >
                                {!openDisputes?.length ? (
                                    <p className="rounded-md border border-dashed border-border/80 bg-muted/20 px-4 py-10 text-center text-sm text-muted-foreground">
                                        No open disputes.
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            {openDisputes.map((row) => (
                                                <TableRow key={row.id}>
                                                    <TableCell className="font-medium">{row.order_number ?? `#${row.id}`}</TableCell>
                                                    <TableCell>
                                                        <StatusBadge status={row.status} />
                                                    </TableCell>
                                                    <TableCell className="text-muted-foreground">{fmtDate(row.opened_at)}</TableCell>
                                                </TableRow>
                                            ))}
                                        </TableBody>
                                    </Table>
                            ...
                            <DashboardQueuePanel
                                ...
                                description="Requested or under review."
                                href={links.withdrawals}
                                locked={!sectionAccess.pending_withdrawals}
                            >
                                            ...
                                            {pendingWithdrawals.map((row) => (
                                                <TableRow key={row.id}>
                                                    ...
                                                    <TableCell className="tabular-nums">
                                                        {fmtMoney(row.requested_amount, row.currency)}
                                                    </TableCell>
                                                    <TableCell>
                                                        <StatusBadge status={row.status} />
                                                    </TableCell>
                            ...
                            <DashboardQueuePanel
                                ...
                                href={links.sellers}
                                locked={!sectionAccess.seller_verification_queue}
                            >
                                {!sellerVerificationQueue?.length ? (
                                    <p className="rounded-md border border-dashed border-border/80 bg-muted/20 px-4 py-10 text-center text-sm text-muted-foreground">
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            {sellerVerificationQueue.map((row) => (
                                                <TableRow key={row.id}>
                                                    <TableCell className="max-w-[160px] truncate font-medium">
                                                        {row.seller_display_name ?? '—'}
                                                    </TableCell>
                                                    ...
                                                    <TableCell className="text-right">
                                                        {row.workspace_url ? (
                                                            <Button variant="outline" size="sm" asChild>
                                                                <Link href={row.workspace_url}>Open</Link>
                                                            </Button>
                                                        ) : (
                                                            '—'
                                                        )}
                            ...
                            <DashboardQueuePanel
                                ...
                                description="Drafts and inactive listings surfaced for review."
                                href={links.products}
                                locked={!sectionAccess.product_moderation}
                            >
                                {!productModeration?.length ? (
                                    <p className="rounded-md border border-dashed border-border/80 bg-muted/20 px-4 py-10 text-center text-sm text-muted-foreground">
                                        ...
                                        <TableBody>
                                            {productModeration.map((row) => (
                                                <TableRow key={row.id}>
                                                    ...
                                                    <TableCell className="text-muted-foreground">{fmtDate(row.updated_at)}</TableCell>
                                                </TableRow>
                                            ))}
                                        </TableBody>
                                    </Table>
                                ...
                                <Card key={i} className="border-border/80 bg-card shadow-sm">
                                    ...
                                    {a.href ? (
                                        <CardContent className="pt-0">
                                            <a href={a.href} className="text-sm font-medium text-primary hover:underline">
                                                Open
                                            </a>
                                        </CardContent>
                                    ) : null}
                ...
                <TabsContent value="layout" className="mt-6">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Stats use one aggregate query; queue tables use bounded selects with constrained eager loads.
                                Financial                                 totals sum raw decimals across currencies — add FX or a reporting currency dimension
                                before board-level financials.
                            </CardDescription>
                        </CardHeader>
                    </Card>
                </TabsContent>
