...
import { ChevronDown, CircleDollarSign, Filter, LockKeyhole, Search } from 'lucide-react';
...
function initialForOrder(order) {
    const normalized = String(order || '').replace(/[^a-zA-Z0-9]/g, '').trim();
...
export default function EscrowsIndex({ header, rows, pagination, filters, index_url, state_options }) {
    ...
    const applyFilters = (next = {}) => {
        ...
        if (!params.state || params.state === 'all') delete params.state;
        router.get(index_url, params, { preserveState: true, preserveScroll: true, replace: true });
    };

    const toggleAll = (checked) => {
        setSelected(checked ? new Set((rows || []).map((row) => String(row.order ?? row.href))) : new Set());
    };
    ...
    const deleteEscrow = (row) => {
        if (!row?.delete_href) return;
        const label = String(row.order ?? 'this escrow');
        if (!window.confirm(`Delete escrow for ${label}? This action cannot be undone.`)) return;
                        ...
                        >
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    placeholder="Search escrows by ID or order..."
                        ...
                        <table className="min-w-full border-collapse">
                            <thead>
                                <tr className="h-[45px] border-b border-slate-200 bg-slate-50/80 dark:border-slate-700 dark:bg-slate-900/30">
                                    ...
                                    <th className="min-w-[240px] px-3 text-left text-[10px] font-bold uppercase tracking-normal text-slate-500 dark:text-slate-400">Order</th>
                                    <th className="min-w-[180px] px-3 text-left text-[10px] font-bold uppercase tracking-normal text-slate-500 dark:text-slate-400">Escrow</th>
                                    <th className="min-w-[140px] px-3 text-left text-[10px] font-bold uppercase tracking-normal text-slate-500 dark:text-slate-400">State</th>
                                    <th className="min-w-[160px] px-3 text-left text-[10px] font-bold uppercase tracking-normal text-slate-500 dark:text-slate-400">Held</th>
                                ...
                                {rows?.length === 0 ? (
                                    <tr>
                                        <td colSpan={7} className="px-4 py-16 text-center text-sm text-slate-500 dark:text-slate-400">
                                            No escrow accounts found for the current filters.
                                        </td>
                                    </tr>
                                ) : (
                                    (rows || []).map((row, index) => {
                                        const id = String(row.order ?? row.href ?? index);
                                            ...
                                            >
                                                ...
                                                <td className="px-3 align-middle">
                                                    <div className="flex items-center gap-3">
                                                        <div className={cn('flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[11px] font-bold', avatarTone(order))}>
                                                            {initialForOrder(order)}
                                                        </div>
                                                        <Link href={String(row.order_href || '#')} className="block truncate text-[13px] font-semibold text-slate-950 transition-colors hover:text-primary dark:text-slate-100">
                                                            {order}
                                                        </Link>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="px-3 align-middle">
                                                    <StatusBadge status={String(row.state ?? '')} className="px-2.5 py-1 text-[10px]" />
                                                </td>
                                                <td className="px-3 align-middle">
                                                ...
                                                <td className="px-3 text-right align-middle">
                                                    <DropdownMenu>
                                                        ...
                                                        <DropdownMenuContent align="end" className="min-w-[180px] rounded-lg">
                                                            <DropdownMenuLabel className="px-2 py-1.5 text-[11px] uppercase tracking-normal text-slate-500">Escrow actions</DropdownMenuLabel>
                                                            <DropdownMenuItem asChild className="rounded-md text-[13px]">
                                                                <Link href={String(row.href || '#')}>View escrow details</Link>
                                                            </DropdownMenuItem>
                                                            <DropdownMenuItem asChild className="rounded-md text-[13px]">
                                                                <Link href={String(row.order_href || '#')}>View order</Link>
                                                            </DropdownMenuItem>
                                                            <DropdownMenuSeparator />
                                                            <DropdownMenuItem className="rounded-md text-[13px] text-red-600 focus:text-red-600 dark:text-red-400 dark:focus:text-red-400" onSelect={() => deleteEscrow(row)}>
                                                                Delete escrow
                                                            </DropdownMenuItem>
                                                        </DropdownMenuContent>
                                                    </DropdownMenu>
