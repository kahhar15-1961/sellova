...
class _SignInGateScreenState extends ConsumerState<SignInGateScreen> {
    ...
    try {
      ...
      if (token == null || token.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text(
                  'Google sign-in was cancelled or is not configured on this device.')),
        );
        return;
      }
      await ref
          .read(authSessionControllerProvider.notifier)
          .loginWithGoogleIdToken(idToken: token);
    } catch (error) {
      if (!mounted) {
        return;
      }
      final message = switch (error) {
        ApiException() =>
          error.message.isNotEmpty ? error.message : 'Google sign-in failed.',
  ...
  Future<void> _apple() async {
    ...
    if (!appleOk) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Sign in with Apple is available on iOS and macOS.')),
      );
      return;
    }
    setState(() {
    ...
    try {
      final (String? idToken, String? email) =
          await AuthSocialSignIn.appleCredentials();
      if (!mounted) {
        return;
      }
      ...
      final message = switch (error) {
        ApiException() =>
          error.message.isNotEmpty ? error.message : 'Apple sign-in failed.',
        _ =>
          'Apple sign-in failed. Set APPLE_CLIENT_ID on the server to your Services ID.',
      };
      setState(() {
        _errorMessage = message;
      });
  ...
  Widget build(BuildContext context) {
    ...
    return AuthLandingShell(
      title: 'Welcome back',
      subtitle:
          'Log in to continue shopping, chatting, and tracking orders from anywhere.',
                  ...
                  children: <Widget>[
                    ...
                    Expanded(
                      child: Text(
                        _errorMessage!,
                        style:
                            TextStyle(color: Colors.red.shade900, height: 1.35),
                      ),
                    ),
                  ],
                ),
